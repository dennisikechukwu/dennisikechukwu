...
const Skill = () => {
  return (
    <section id="skills" className="mt-3 bg-black p-7 rounded-lg">
      <div className='mt-4 lg:mt-7 p-3'>
        <h2 className="lg:text-3xl text-xl font-bold mb-4 text-center text-[white] font-sans">
          I build engaging, high performance web applications.
        </h2>
        <p className="text-[#cecdcd] mb-8 text-center font-sans">
          Leveraging modern front-end technologies to craft seamless user experiences.
        </p>
      </div>
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
        {skills.map((skill, index) => (
          <div key={index} className="bg-[#0e0e0e] p-6 rounded-lg flex flex-col items-start border-[0.8px] border-[#323232] hover:border-[#494949] transition-all">
            <div className="text-white mb-4 text-xl">{skill.icon}</div>
            <h3 className="text-xl font-semibold mb-2 text-[#f0eaea] font-sans ">{skill.title}</h3>
